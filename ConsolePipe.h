...
public:
	CConsolePipe(
		...
		CEdit* wOutput,
		DWORD flags
		) : m_bIsNT(bWinNT), m_dwFlags(flags)
	{
		m_wndOutput = wOutput;
/*
		if(wOutput->m_hWnd) {
			//ATLASSERT(lstrcmpi(wOutput.GetClassName(), _T("EDIT")) == 0); // richedit is ok too
...
*/
	...
	~CConsolePipe()
	{
		OutputDebugString(_T("~CConsolePipe()\n"));
		SendCtrlBrk();
		Cleanup();
	}
	...
	int Execute(LPCTSTR pszCommand)
	{
		...
		if(CreatePipe(&loadzaHandles[0], &loadzaHandles[1], &sa, 0) &&
			...
			CloseHandle(loadzaHandles[0]);
			loadzaHandles[0] = INVALID_HANDLE_VALUE;
			CloseHandle(loadzaHandles[2]);
			loadzaHandles[2] = INVALID_HANDLE_VALUE;
			...
			if(LaunchRedirected(pszCommand, loadzaHandles[1], loadzaHandles[3], loadzaHandles[4])) {
				...
				for(int idx = 0; idx < sizeof(loadzaHandles) / sizeof(loadzaHandles[0]); idx++) {
					if(loadzaHandles[idx] != INVALID_HANDLE_VALUE) {
						CloseHandle(loadzaHandles[idx]);
						loadzaHandles[idx] = INVALID_HANDLE_VALUE;
					}
				}
	...
	virtual void OutputToWiindow(LPCTSTR pszText)
	{
...
protected:
	...
	HANDLE m_hChildThread; // the one we launch
